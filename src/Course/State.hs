...
exec ::
  ...
  -> s
  -> s
exec sa s = let (a, s) = runState sa s in s
...
eval ::
  ...
  -> s
  -> a
eval sa s = let (a, s) = runState sa s in a
...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b
  (<*>) f sa = State (\s -> let (a, s) = runState f s 
                                (a', s') = runState sa s                             in (a a', s')) 
...
findM ::
  ...
  -> f (Optional a)
findM f  =
  foldRight (\x ys -> lift2 (\z oa -> if  z then Full x else oa)  (f x) ys) (pure Empty) 
